...
if (VIDEOS.length === 2) {
  console.log("ez playback enabled on echo360");
  ...
  document.addEventListener("keydown", (ev) => {
    ...
    if (setEqual(current_pressed_keys, KEY_INCREASE_SPEED)) {
      const index = PLAYBACK_SPEED.indexOf(VIDEOS[0].playbackRate);
      if (index == PLAYBACK_SPEED.length - 1) {
        return;
      }
      current_playback_speed = PLAYBACK_SPEED[index + 1];
      VIDEOS[0].playbackRate = current_playback_speed;
      VIDEOS[1].playbackRate = current_playback_speed;
